"""
Generate 3-panel top-surface visualization: FEA | PINN | Abs Error
Matches the exact format of the reference figure.
...
def main():
    ...
    # Extract top surface (z = H)
    mask = np.isclose(zg, H, atol=1e-6)
    x_top = xg[mask]
    y_top = yg[mask]
    u_fem_top = u_fem[:,:,:,2][mask]
    
    # PINN prediction on ALL grid points (then extract top)
    X = xg.ravel()
    Y = yg.ravel()
    Z = zg.ravel()
    ...
    u_pinn_top = u_pred_reshaped[:,:,:,2][mask]
    
    # Absolute error
    abs_err = np.abs(u_pinn_top - u_fem_top)
    mae_pct = 100 * np.mean(abs_err) / np.max(np.abs(u_fem_top))
    ...
    points = np.column_stack([x_top, y_top])
    U_fem = scipy.interpolate.griddata(points, u_fem_top, (Xi, Yi), method='cubic')
    U_pinn = scipy.interpolate.griddata(points, u_pinn_top, (Xi, Yi), method='cubic')
    U_err = scipy.interpolate.griddata(points, abs_err, (Xi, Yi), method='cubic')
    ...
    # Panel 1: FEA
    im1 = axes[0].imshow(U_fem, extent=[0, 1, 0, 1], origin='lower',
                         aspect='equal', cmap='jet', vmin=vmin, vmax=vmax)
    axes[0].set_title(f'Three-Layer FEA\nE=[{E1:.1f},{E2:.1f},{E3:.1f}], t={H:.3f}', fontsize=12)
    axes[0].set_xlabel('x')
    axes[0].set_ylabel('y')
    plt.colorbar(im1, ax=axes[0], fraction=0.046, pad=0.04)
    
    # Panel 2: PINN
    im2 = axes[1].imshow(U_pinn, extent=[0, 1, 0, 1], origin='lower',
                         aspect='equal', cmap='jet', vmin=vmin, vmax=vmax)
    axes[1].set_title('Three-Layer PINN', fontsize=12)
    axes[1].set_xlabel('x')
    axes[1].set_ylabel('y')
    plt.colorbar(im2, ax=axes[1], fraction=0.046, pad=0.04)
    
    # Panel 3: Absolute Error
    im3 = axes[2].imshow(U_err, extent=[0, 1, 0, 1], origin='lower',
                         aspect='equal', cmap='magma', vmin=0)
    axes[2].set_title(f'Abs Error\nMAE={mae_pct:.2f}%', fontsize=12)
    axes[2].set_xlabel('x')
    axes[2].set_ylabel('y')
    plt.colorbar(im3, ax=axes[2], fraction=0.046, pad=0.04)
    
    plt.tight_layout()
    output_path = OUTPUT_DIR / 'three_layer_top_surface_comparison.png'
